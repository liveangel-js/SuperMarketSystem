...
public class CommidityData implements IDataReader,IDataWriter,IDataUpdate{
	
	ArrayList<CommidityRecord> commidityList = new ArrayList<CommidityRecord>();
	String fileName = "goods.txt";
	
	public CommidityData(){
		File file = new File(fileName);
        try{
            BufferedReader bufferReader = new BufferedReader(new FileReader(file));
            String text = "";
            String[] goodInfo = null;
            CommidityRecord cr = null;
            while((text = bufferReader.readLine())!=null){
            	if(text.trim().equals(""))
            		continue;
                goodInfo = processInfo(text);
                cr = new CommidityRecord(Integer.parseInt(goodInfo[0]),goodInfo[1],Double.parseDouble(goodInfo[2]),Double.parseDouble(goodInfo[3]),goodInfo[4],goodInfo[5]);
	...
	public Record readDataByID(int id) {
		...
		for(CommidityRecord c : commidityList){
			if(c.getGid() == id)
				return c;
		}
		return null;
