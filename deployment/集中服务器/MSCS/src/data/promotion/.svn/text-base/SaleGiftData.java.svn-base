...
public class SaleGiftData extends Data {
	...
	public void dealSale(SaleRecord sale){
		double price=0;
		//SaleGiftRecord discount=null;
		for(int i=0;i<list.size();i++){
			if(sale.getSpecialPrice()>=list.get(i).getPrice()&&list.get(i).getPrice()>=price&&list.get(i).isDuringTime()){
				//discount=list.get(i);
				price=list.get(i).getPrice();
			}
		}
		for(int i=0;i<list.size();i++){
			if(list.get(i).getPrice()==price&&list.get(i).isDuringTime()){
			    list.get(i).dealSale(sale);
			}
		}
//		if(discount!=null){
